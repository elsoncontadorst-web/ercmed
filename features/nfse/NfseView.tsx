...
const NfseView: React.FC = () => {
    ...
    const draft = useMemo<NfseDraft>(() => ({
        ...
        provider: {
            ...
            simpleNationalOption: profile.regime === 'mei' ? 2 : 3,
            simpleNationalTaxRegime: profile.regime === 'simples' ? profile.simpleNationalTaxRegime || 1 : undefined,
            specialTaxRegime: 0
        },
        ...
        service: {
            ...
            issRate: profile.regime === 'mei' ? undefined : profile.issRate,
    ...
    return <div className="min-h-full bg-slate-50 p-4 md:p-6">
        <div className="mx-auto max-w-6xl space-y-5">
            <header className="overflow-hidden rounded-3xl bg-gradient-to-r from-slate-950 via-slate-900 to-teal-900 p-7 text-white shadow-xl">
                <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
                    <div><span className="inline-flex items-center gap-2 rounded-full bg-teal-400/15 px-3 py-1 text-xs font-black uppercase tracking-wider text-teal-200"><FileCheck2 size={14}/> Emissor Nacional</span><h1 className="mt-3 text-3xl font-black">Emissor NFS-e</h1><p className="mt-2 max-w-2xl text-sm text-slate-300">Emita notas de serviços da clínica com certificado A1, validação prévia e histórico por unidade.</p></div>
