...
const CallPanelControl: React.FC<Props> = ({ ownerId, clinicId, clinicName, appointments, professionals }) => {
  ...
  const launchWindowsTvMode = () => {
    if (!ownerId) return;
    const panelId = getCallPanelId(ownerId, clinicId);
    const tvUrl = youtubeUrl.trim() || 'https://globoplay.globo.com/tv-globo/ao-vivo/6120663/';
    const youtubeSource = /(^|\.)youtube\.com|youtu\.be/i.test(tvUrl);
    const panelUrl = `${window.location.origin}/painel/${encodeURIComponent(panelId)}${youtubeSource ? '' : '?overlay=1'}`;
    const command = `ercmedtv://start?panelUrl=${encodeURIComponent(panelUrl)}&tvUrl=${encodeURIComponent(tvUrl)}&mode=${youtubeSource ? 'integrated' : 'overlay'}`;
  ...
  return <section className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm print:hidden">
    <div className="mb-4 flex flex-col justify-between gap-3 lg:flex-row lg:items-center">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        ...
        <a href="/downloads/ERCMed-TV-Setup-1.0.4.exe" download className="flex items-center justify-center rounded-xl border border-teal-600 bg-white px-4 py-3 text-xs font-bold text-teal-700 hover:bg-teal-50">Instalar ERCMed TV</a>
