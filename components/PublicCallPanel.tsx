...
const PublicCallPanel: React.FC = () => {
  ...
  return <main className="min-h-screen overflow-hidden bg-[#071c24] text-white">
    <header className="flex min-h-20 items-center justify-between gap-4 border-b border-white/10 bg-[#0b2932] px-5 py-3 lg:px-7">
    ...
    <div className="grid h-[calc(100vh-5rem)] grid-cols-1 lg:grid-cols-[minmax(0,3fr)_minmax(300px,1fr)]">
      <section className="relative hidden items-center justify-center overflow-hidden bg-gradient-to-br from-[#0a3039] via-[#0b2630] to-[#07181f] p-10 lg:flex">
        <div className="absolute inset-0 opacity-20 [background-image:radial-gradient(circle_at_20%_20%,#2dd4bf_0,transparent_28%),radial-gradient(circle_at_80%_75%,#38bdf8_0,transparent_25%)]"/>
        {callIsFresh ? <div className="relative w-full max-w-4xl animate-pulse rounded-[2rem] border border-teal-300/40 bg-teal-400/10 p-12 text-center shadow-2xl shadow-teal-950/50"><p className="mb-4 text-2xl font-black uppercase tracking-[0.35em] text-teal-300">Chamando</p><p className="text-[clamp(6rem,16vw,13rem)] font-black leading-none tracking-tight">{panel?.currentCall?.ticketNumber}</p><p className="mt-7 text-[clamp(2rem,5vw,4rem)] font-extrabold text-amber-300">{panel?.currentCall?.destination}</p>{panel?.currentCall?.professionalName && <p className="mt-3 text-2xl text-slate-200">{panel.currentCall.professionalName}</p>}</div> : <div className="relative max-w-3xl text-center"><MonitorUp className="mx-auto mb-7 h-24 w-24 text-teal-300/80"/><h2 className="text-5xl font-black">Aguarde sua senha ser chamada</h2><p className="mt-5 text-2xl text-slate-300">Acompanhe as chamadas no painel ao lado.</p><p className="mt-10 rounded-2xl border border-white/10 bg-white/5 px-6 py-4 text-base text-slate-400">Para assistir à TV ao mesmo tempo, deixe o Globoplay aberto ao lado desta janela no segundo monitor.</p></div>}
      </section>
      <aside className="relative overflow-y-auto border-l border-white/10 bg-[#0b252e] p-4 lg:p-6">
