...
const PublicCallPanel: React.FC = () => {
  ...
  return <main className={`min-h-screen overflow-hidden text-white ${overlayMode ? 'bg-transparent' : 'bg-[#071c24]'}`}>
    <header className={`flex min-h-20 items-center justify-between gap-4 border-b border-white/10 px-5 py-3 lg:px-7 ${overlayMode ? 'ml-[72%] bg-[#0b2932]/95' : 'bg-[#0b2932]'}`}>
    ...
    <div className={`grid h-[calc(100vh-5rem)] grid-cols-1 ${overlayMode ? 'lg:grid-cols-[minmax(0,72fr)_minmax(300px,28fr)]' : 'lg:grid-cols-[minmax(0,3fr)_minmax(300px,1fr)]'}`}>
      <section className={`relative hidden items-center justify-center overflow-hidden p-10 lg:flex ${overlayMode ? 'bg-transparent' : 'bg-gradient-to-br from-[#0a3039] via-[#0b2630] to-[#07181f]'}`}>
      ...
      <aside className={`relative overflow-y-auto border-l border-white/10 p-4 lg:p-6 ${overlayMode ? 'bg-[#0b252e]/95' : 'bg-[#0b252e]'}`}>
